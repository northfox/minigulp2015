...
gulp.task('script:_makeEntries', () => {
  if (minigulp.option.dev === false) {
    delete conf.devtool;
  }
  return gulp.src(config.path.js.src).pipe(
    through.obj(function(file, charset, callback) {
...
function executeWebpack(watch, callback) {
  conf.watch = watch;
  conf.output.path = minigulp.getPath('js', 'dest');
